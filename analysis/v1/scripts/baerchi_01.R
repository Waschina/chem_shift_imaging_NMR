...
tmp <- fread("data/raw/baerchi_points_all")
...
# # ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ #
# baer_HL <- list()
# baer_HL$color <- cbind(baer_H$color[,.(sampleH = sample, color)],
#                        baer_L$color[,.(sampleL = sample)],
#                        baerchi$info[,.(newID,rack)])
# baer_HL$filled <- rbind(merge(baer_H$filled, baer_HL$color[,.(sample = sampleH,
#                                                               newID)],
#                               by = "sample"),
#                         merge(baer_L$filled, baer_HL$color[,.(sample = sampleL,
#                                                               newID)],
#                               by = "sample"))
# baer_HL$filled$mode <- c(rep("H",nrow(baer_H$filled)),
#                          rep("L",nrow(baer_L$filled)))
# baer_HL$filled[, peakIndexMode := paste0(mode,"-", peakIndex)]
# tmpH <- baer_H$feat.tab; colnames(tmpH) <- paste0("H-",colnames(tmpH))
# tmpL <- baer_L$feat.tab; colnames(tmpL) <- paste0("L-",colnames(tmpL))
# baer_HL$feat.tab <- cbind(tmpH,tmpL)
# rm(tmpH); rm(tmpL)
# tmp_peaks.features <- dcast(baer_HL$filled, newID ~ peakIndexMode,
#                             value.var = "peakValue", fill = 0)
# setkey(tmp_peaks.features, "newID")
# tmp_peaks.features <- tmp_peaks.features[baer_HL$color$newID]
# tmp_peaks.features <- as.matrix(tmp_peaks.features[,-1])
# rownames(tmp_peaks.features) <- baer_HL$color$newID
...
# keep_feat <- baer_HL$filled[!(ppm < 0.4 | (ppm > 4.41 & ppm < 6)), unique(peakIndexMode)]
...
#                 ft.pca$x)
# dt.pca <- merge(dt.pca, baer_HL$color)
# dt.pca.loading <- cbind(data.table(feat = rownames(ft.pca$rotation)),
#                         ft.pca$rotation)
# dt.pca.loading <- merge(dt.pca.loading, baer_HL$filled[,.(ppm = median(ppm),
...
#
# pairs(dt.pca[,2:6],
#       col = as.factor(dt.pca$color), main = "Bärchi (HL)")
# pairs(dt.pca[,2:6],
#       col = as.factor(dt.pca$rack), main = "Bärchi (HL)")
#
#
#
...
#
#   p_scores <- ggplot(dt.pca, aes(get(PC_OI[1]), get(PC_OI[2]), fill = color)) +
#     geom_point(shape = 21) +
...
# dt_scaled <- merge(dt_scaled, baerchi$info, by = "newID")
# dt_scaled[, pos.y.new := paste0(rack,"-",pos.y)]
# dt_scaled$pos.y.new <- factor(dt_scaled$pos.y.new,
#                               levels = rev(dt_scaled[order(rack, pos.y, pos.x), unique(paste0(rack,"-",pos.y))]))
#
#
# ggplot(dt_scaled[feat == "H-13486"], aes(x = pos.x, y = pos.y.new,
#                                          fill = signal)) +
#   geom_tile()
#
#
# dt_pca_baer <- copy(dt.pca[,1:10])
# dt_pca_baer <- merge(dt_pca_baer, baerchi$info, by = "newID")
# dt_pca_baer[, pos.y.new := paste0(rack,"-",pos.y)]
# dt_pca_baer$pos.y.new <- factor(dt_pca_baer$pos.y.new,
#                                 levels = rev(dt_pca_baer[order(rack, pos.y, pos.x), unique(paste0(rack,"-",pos.y))]))
#
#
# ggplot(dt_pca_baer, aes(x = pos.x, y = pos.y.new,
#                         fill = PC1)) +
#   geom_tile()
#
